...
JD_TEMPLATE_INSTRUCTION = """
【严格的 JD 格式要求】
输出的 `enhanced_jd` 字段**必须严格遵循**以下模板（标记符号和分段层级不能省略）：
...
2. **补充** — 基于行业参考或联网信息，补充该岗位常见的职责和要求
3. **市场对标** — 标注每条要求是"必备"还是"加分"
4. **行业洞察** — 如果提供了联网搜索信息，请基于它们补充行业趋势和市场数据
...
{{
    "enhanced_jd": "（严格按上方【严格的 JD 格式要求】生成完整 JD 文本，必须和原始 JD 显著不同）",
    ...
    return {
        "enhanced_jd": f"""## 职位名称：{raw_jd.strip()[:30]}
